...
namespace Searchname.Services
{
    public class DataService : IDataService
    {
        ...
        private async Task<IList<Models.Result>> SearchName(string name)
        {
            ...
            {
                var text = await client.GetStringAsync(Utilities.Constants.NameSearchEndpoint + name.ToLower());
                ...
                foreach (var result in nameSearchData.ani)
                {
                    ...
                    if (locationResolveData?.address?.country == "România" && locationResolveData?.address?.county is not null)
                    {
